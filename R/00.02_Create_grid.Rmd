...
Create grid of 1 km x 1 km to fit inside the 5 km x 5 km one in use by CWS Atl contained in study_area.
...
# Rasterize: fill with an attribute column telling which 5x5 km grid cell it 
# came from.
gengrid <- rasterize(grid_vect, r, field = "ET_ID")
...
# Now rasterize again filling with the 1x1 km grid id since we'll want
# that as well
idgrid <- rasterize(grid_vect, r, field = "ID")

# Save as a raster
writeRaster(idgrid,
            filename = file.path(RasterDir, "idgrid.tif"),
            overwrite = TRUE)
