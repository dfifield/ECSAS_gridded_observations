...
# filter out any with missing date or location
ecsas.air.dat.on.effort <- select(ecsas.air.dat.on.effort, -ObsTime) %>% 
  filter(!is.na(LatStart), !is.na(LongStart), !is.na(Date))
...
Left in for posterity but doesn't actually get used since I aggregate by watchID
and these don't actually have watchIDs (although I assign fake ones). However, I am
(currently) using watchIDs gleaned from ECSAS_aerial_watches.rda from Atlantic DSM
analysis.
...
study.area <- watches.sf %>%
  ...
  study.area <- st_read(file.path(ShapeDir, "GRID_5km.shp"))
} else 
  stop("00.01_Extract_data.Rmd: Unknown SubProject ", SubProject)
